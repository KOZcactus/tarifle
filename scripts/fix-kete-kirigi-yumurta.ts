...
async function main() {
  ...
  const info = assertDbTarget("fix-kete-kirigi-yumurta");
  ...
  try {
    const r = await prisma.recipe.findUnique({
      where: { slug: "kete-kirigi-tava-ardahan-usulu" },
    ...
    if (r.allergens.includes("YUMURTA")) {
      console.log("already has YUMURTA:", r.allergens);
      return;
    }
    const next = Array.from(new Set([...r.allergens, "YUMURTA" as Allergen])).sort();
