...
> Son güncelleme: **Oturum 24 SONU (27 Nis 2026), 22+ commit.** **Mod K (Tarif Kontrol) tam paket** (brief §20 yeni mod + 3 script prepare/verify/apply + 71 input dosyası önceden üretildi + Batch 1a-4a apply 122 correction + 5 MAJOR_ISSUE inc. currywurst vegan tag, biscuit-gravy vejetaryen+sosis, bo-la-lot lá lốt yaprağı, bobo-de-camarao + boeuf-bourguignon cuisine fix manuel). **Personal "Pişirdiklerim" anasayfa shelf + a11y fix** (FeaturedShelf ariaLabel parametrik, Personal cooked + Bu hafta en çok pişirilenler doğru duyurma, profil sayfasında pisirdiklerim anchor). **Mod M Batch 1-3 revert + Batch 1 yeniden apply 36 marine** (kalite TAM TR + redundancy fix, brief §19'a Kural 7+8 eklendi, ilk Batch'lerde 52/57 ASCII fold sorunu kapandı). **butter-chicken Delhi merge** (Mod IB silinen Delhi otantik içerik global butter-chicken'a transfer, AuditLog DELHI_BUTTER_MERGE). **3 yeni blog (52→55)**: Yumurta Tazeliği ve Saklama (malzeme-tanima, USDA + FDA + 4 kaynak), Tahıl Çeşitleri Karşılaştırma (malzeme-tanima, 5×8 + 8×3 table, 8 kaynak), Çorba Bilimi: Tabandan Servise (pisirme-teknikleri, 10×4 çorba teknik haritası table, 6 kaynak). **MDX remark-gfm desteği** (npm install + MDXRemote options.mdxOptions.remarkPlugins, tüm bloglar table syntax + strikethrough + autolink). **Cross-language pair audit yeniden** (38 pair, hepsi varyant, 0 sil; Mod IB pass etkili). **CODEX_NEW_CHAT_INTRO Bölüm 1 + Bölüm 6 Mod K tetik şablonu eklendi.** Detay: `docs/PROJECT_STATUS.md` → "Oturum 24 SONU (27 Nis 2026)".
