/**
 * Fix CRITICAL allergen gaps surfaced by v2 audit-deep (normalize + inflected
 * keyword support). Each recipe already has other allergens; we UNION-add
 * the missing one.
 *
 *   npx tsx scripts/fix-critical-allergens-v2.ts              # dry run
 *   npx tsx scripts/fix-critical-allergens-v2.ts --apply      # write
 */
...
const FIXES: readonly { slug: string; add: Allergen[]; reason: string }[] = [
  { slug: "kasarli-tost", add: ["GLUTEN"], reason: "Tost ekmeği" },
  { slug: "filmjolk-meyveli-smoothie", add: ["GLUTEN"], reason: "Yulaf" },
];
...
async function main(): Promise<void> {
  assertDbTarget("fix-critical-allergens-v2");
  console.log(
    `🔧 fix-critical-allergens-v2 (${APPLY ? "APPLY" : "DRY RUN"}) → ${
