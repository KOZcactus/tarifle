...
const slugs = [
  ...
  "taze-fasulyeli-bulgur-pilavi",
  "tepsi-orugu-adiyaman-usulu",
  "tay-feslegenli-patlican",
  "tavuklu-mantarli-pie-tava-ingiltere-usulu",
];
...
async function main() {
  ...
  for (const slug of slugs) {
    ...
    console.log("\n=== " + slug + " ===");
    console.log("title:", r.title, "| cuisine:", r.cuisine, "| type:", r.type, "| diff:", r.difficulty,
      "| min:", r.prepMinutes + "/" + r.cookMinutes + "/" + r.totalMinutes, "| kcal:", r.averageCalories);
    console.log("desc:", r.description);
    console.log("tipNote:", r.tipNote);
    console.log("servingSuggestion:", r.servingSuggestion);
    console.log("ingredients:");
    r.ingredients.forEach((i) => console.log("  - " + i.name + " | " + i.amount + " " + i.unit));
    console.log("steps:");
    r.steps.forEach((s) => console.log("  " + s.stepNumber + " [" + (s.timerSeconds ?? "-") + "] " + s.instruction.slice(0, 130)));
  }
