...
> Son güncelleme: **Oturum 18 sonu (24 Nis 2026), 30 commit.** **3452 tarif prod** (3252 → +200 Mod A 35a/35b/36a/36b). **AI paketi A-I 9 özellik** (A Pişirdim→decrement + B TTS kadın/erkek sesli tarif okuma + C SKT widget + D dinamik zaman banner + E Beğenmedim feedback + F v3 miktar rozeti + G favori boost + H home 🎒 CTA + I benzer filter chip). **Mod F retrofit pipeline 6/27** (B+ → A- → A+ → A → A- → A+, brief 5 gate A+ standardı). **Mod B Backfill-11/12/13 apply** (+250 çeviri, ~%93 tam). **Backfill-14/15 CSV üretildi** (200 slug Codex'te). **Pantry miktar döngüsü tam** (quantity match + rozet + SKT opt-in + alışveriş→pantry + Pişirdim→decrement + 45+10 unit test). **Brief A+ standardı** (§5 Mod A + §15 Mod F, 5 self-check gate, kritik nokta %60 zorunlu). **TTS voice pref** (User.ttsVoicePreference, kadın default + erkek toggle). **CI fix dalgası** (pantry test vi.hoisted + content:validate staple WARNING + em-dash kod yorumu temizliği). **752/752 unit test PASS** (674 + 78 yeni), **27 migration**. Detay: `docs/PROJECT_STATUS.md` → "24 Nisan 2026 (oturum 18)".
