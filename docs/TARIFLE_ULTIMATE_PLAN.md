...
> Son güncelleme: **23-24 Nisan 2026** (oturum 16 sonu, 32 commit, AI v4 + Mod B %100 + Admin UX)
> Durum: MVP + Faz 2 + Faz 3 **canlıda** (**3021 tarif prod**, **Mod B %100 kapanış** (3021/3021 tam çevirili), **AI Asistan v4 TAM SHIP** (haftalık menü planlayıcı + shopping entegrasyonu + macro preference + plan favorites + pantry history + autocomplete + preset chips + person count scaling), **Admin Tarif Düzenle v3** (@dnd-kit drag-drop + çift onay modal + canlı preview pane), **Mod E pipeline kapandı** (B1-B29 = 2900 tarif revize), **30 blog yazısı** (kategori 12/9/9 ideal denge), **5 Codex modu aktif (A/B/C/D/E)** + **Mod A 50/50 yarı batch pattern** (30a/30b, 31a/31b, 32a + 32b STUB stash'te), **Neon Vercel Marketplace migration** ($240/yıl tasarruf), **Vercel Analytics aktif**, **Tarif basit/lüks varyant paneli**, **PWA install events Sentry → Vercel**, Hero A/B durduruldu default A, Tailwind 4 dark variant fix, Faz 1 Leaderboard, Privacy 3 toggle, Newsletter prod, Sentry Replay, **Prod audit-deep: RESULT PASS 0 CRITICAL**, WCAG 100/100, Pre-push 5 katman, **648/648 test PASS**, Cache TTL agresif)
> Versiyon: 2.0
...
>
> **22 Nisan 2026 oturum 12 notları, GPT dış audit response + Faz 1 topluluk altyapısı (~29 commit)**: GPT 5.4 Pro dış audit'inde P0 sorunlar (PDF Türkçe font + web↔PDF rounding + diyet safety + cuisine sayaç + canonical gap + ItemList JSON-LD + alkol noindex kaldır + viewCount threshold + AgeGate overlay) tek seansla kapandı. **Home iki hero**: primary "Bugün ne pişirsek?" + secondary "Evdekilerle ne pişireceğini bul → AI Asistan'a Git" (Seçenek B single CTA, form duplication yok). **Terminoloji revize**: katalog bağlamındaki "tarif" metinleri "yemek çeşidi" (ana sayfa + landing), item-level "tarif" kalır. **Canonical 308 www→apex** + 3 kritik canonical gap (kesfet/hakkimizda/profil). **Batch 27 Mod B prod** (100/100), **Batch 28 Mod A+B** (prod 164 yeni + 100 çeviri), **Batch 29 Mod A+B** (kısmi 45 tarif, `--size N` flag gen-translation-csv.ts). **N+1 query cache** (getRecipeBySlug 30dk + getSimilarRecipes 6sa). **/admin/analytics 42P01 fix** (Prisma `@@map` uyum raw SQL). **Faz 1 Leaderboard**: Prisma migration 7 yeni BadgeKey (EXPERIENCED, PHOTOGRAPHER, CATEGORY_MASTER, EDITOR_CHOICE, WEEKLY/MONTHLY/ALL_TIME_TOP) + `src/lib/leaderboard/score.ts` raw SQL 5 CTE skor util + `/leaderboard` SSR 3 tab + Navbar "Liderlik". **docs/TARIFLE_ULTIMATE_PLAN §35-36** (3 fazlı monetizasyon plan + uygulama kuralları) + **docs/FUTURE_PLANS.md** (yalnız yapılmamış planlar havuzu). **Codex brief §12 Mod C** (Landing SEO copy, 38 item) + **§13 Mod D** (Top 200 editoryal revize). **Vercel Fluid CPU %75** → agresif TTL + docs-only build skip. **Next 16 breaking fix**: `middleware.ts` + `proxy.ts` çakışması kapatıldı (tek proxy.ts). **PPR (cacheComponents) denendi rollback** (paradigm shift 8-12 saat refactor, site açılış sonrasına ertelendi). Pre-push 5 katman sabit, 617/617 test PASS, 21 formal migration. Detay: `docs/PROJECT_STATUS.md` → "21-22 Nisan 2026 (oturum 12)".
>
> **🚨 21 Nisan 2026 gece, oturum 11 kapanış post-mortem (commit `04482f2`)**: Oturum 11 boyunca 36+ commit CI kırmızı gitti, **Vercel prod deploy 0b57f2e öncesi eski commit'te kilitli kaldı**. Kök neden: pre-push 4 katman (lint+validate+em-dash+allergen) vardı ama `tsc --noEmit` yoktu. `scripts/restore-missing-slugs-to-seed.ts` Prisma Decimal→number uyumsuzluk ve `scripts/diff-backfill10-slugs.ts` unknown→string cast hatası lokal'de yakalanmadı. CI'da tsc fail → next build fail → Vercel fail. Fix: `Number()` cast + **pre-push 5. katman tsc --noEmit** eklendi, ~10-15s maliyet ama CI round-trip ve Vercel deploy regression'ı kalıcı olarak önlüyor. DB apply'lar bu süreçte ayrı pipeline'dan prod'a gitti (2553 tarif, Mod B %96.5) ama frontend kitlendi. `04482f2` push sonrası CI yeşile döner, Vercel redeploy, oturum 11 feature'ları (mobile carousel, JSON-LD, content widget, PWA analytics) canlıya çıkar. Detay: `docs/PROJECT_STATUS.md` → "N · CI kırmızı postmortem" bloğu.
>
> **21 Nisan 2026 oturum 11 notları, maraton günü (37 commit)**: **Backfill trilogy 01-13 tam** (Mod B %53 → **%96.5**, ~1266 tarif full EN+DE). **Allergen audit overhaul** (128 over-tag → 0, 20 yeni CRITICAL fix, keyword expansion: GLUTEN +16/SUT +8/DENIZ +10/YUMURTA +7/KUSUYEMIS +4/SUSAM +3). **Source drift trilogy** (553 allergen sync + 34 missing slug restore + 345 content drift sync + format-aware refactor). **Content generator** (+321 tipNote/serving fill + 1069 boilerplate diversify + 1040 v2 refresh, pool 108 → 158 variant). **SEO bakım** (WebSite+Organization JSON-LD + hreflang x-default/tr/en + robots param disallow + sitemap priority tuning + cache TTL %40-50 Neon azalma). **isFeatured boost** (%9.4 → **%12.0** + 63 editör seçimi). **Mobile UX** (home shelf carousel snap+arrow + recipe "🌍 Tam çeviri" badge + AI "Acıktım" polish + PWA engagement gate + install analytics Sentry funnel). **Similar-recipes v3** (cuisine region clustering 8 bölge + region +0.5 + hunger proximity +0.4, 28 test). **Pre-push 4. katman** (allergen source guard scripts/check-allergen-source.ts). **Admin Content Quality widget** (6 metrik %-based). **Sentry noise tune** (client denyUrls + server Auth/Prisma/JWT ignore, %50-60 event azalma). **Batch 27 Mod A prod** (99 new tarif, 4 allergen fix, 2553 total). **Batch 27 Mod B CSV üretildi** (docs/translations-batch-27.csv, Codex'e gönderilmeye hazır). Codex brief kalıcı ders: append noktası mimarisi + recurring block kapanı (batch 27 v1 dersi) + step-ingredient/tag-content drift pattern (backfill-08/09 dersi). Pre-push 4 katman: lint + content:validate + em-dash + allergen. 28 unit test PASS. Detay: `docs/PROJECT_STATUS.md` → "21 Nisan 2026 (oturum 11)".
>
> **20 Nisan 2026 oturum 10 notları, açlık barı + DB kalite turu (18 commit)**: Yeni büyük feature **Açlık barı** (Minecraft-esin 1-10 tokluk, Holt Satiety Index temelli), prod 2454 tarifin hepsinde dolu, 9 UI yüzeyinde görünür (detay + listing kart chip + /tarifler filter+sort "En tok tutan" + AI Asistan "Acıktım" sort + home "Bugünün tarifi" widget + OG image + PDF export + JSON-LD additionalProperty + FAQ "tok tutar mı"). **Batch 24-26 Mod A prod** (+300 Codex tarifi, 25 allergen fix toplamı). **Duplicate merge P3** (155 grup, 166 loser silindi, 0 user-gen reference kaybı, SEO win, sitemap 166 gereksiz URL azaldı). **Mod B backfill altyapısı** (13 CSV × 100, backfill-01 + backfill-02 prod apply = **1399/2454 (%57)**, kalan 11 batch). **Haftalık audit cron** (`/api/cron/audit-report` + Sentry CRITICAL alert, pazartesi 07:00 UTC). **Empty allergen fix** (4 prod + 1 FP rollback coconut istisnası). **E2E TestUser cleanup** + helper cascade fix (Variation/ModerationAction/Report FK onDelete yok, P2003 leak fix). **Tüm manuel env tamam** (Newsletter + Audit + Cloudinary + Pinterest domain claim live, meta tag `p:domain_verify` canlı). **Next 16 deprecation fix**: `middleware.ts` → `proxy.ts`. Codex brief'e "Mod B Backfill default" + "Format ve boş-string kapanı" eklendi. 20 migration (`add_hunger_bar`), 613/613 test PASS. Detay: `docs/PROJECT_STATUS.md` → "20 Nisan 2026 (oturum 10)".
>
> **20 Nisan 2026 oturum 9 notları, altyapı + kalite + maliyet turu (23 commit)**: İçerik yerine altyapı/DX/maliyet optimizasyonuna odaklanıldı. **Mod B batch 21-22-23 prod canlı** (coverage 900 → **1200/2320**), batch 21 üç teslimde geçti (template spam + TR leak + step collapse reddedildi, 3. teslim temiz). Codex brief §9 batch 21 dersleri eklendi. **Daily view log** (`RecipeViewDaily` model + "bu hafta N" chip + admin analytics view trend gerçek mini bar chart). **SEO otomasyonu**: 500 URL'lik search submission listesi + **IndexNow** (middleware + CLI + haftalık Vercel Cron + bulk ping 2301 URL canlı); Bing/Yandex/Seznam otomatik indexlemesi aktif. **Legal humanize**: çerez politikası + güvenlik sayfası teknik terimler (`authjs.session-token`, `bcrypt`, `HSTS`, `Upstash sliding-window`) günlük Türkçeye çevrildi; "sende olanlar / bizde olanlar" → "senin yapabileceklerin / bizim yaptıklarımız"; cookie banner sadeleşti. **Em-dash global yasak** (2500+ karakter temizlendi + `scripts/check-emdash.mjs` pre-push Node guard + punct upgrade 39 yerde `,` → `:` veya `.`); AGENTS.md + CODEX_BATCH_BRIEF §3 yasaklar kuralı aktif. **Cache optimizasyonu**: 5 hot query path'i `unstable_cache` (listing DB hit %90+ azalma, Next.js 16 `updateTag` invalidation). **Neon Launch plan + quota** ($12 hard cap, 385k CU-seconds, API üzerinden set edildi; compute max 1 CU, scale-to-zero 5 dk). 19 formal migration (`add_recipe_view_daily`). 574/574 test PASS, CI 13+ art arda yeşil. Pre-push 3 katman: lint + content:validate + em-dash guard. Detay: `docs/PROJECT_STATUS.md` → "20 Nisan 2026 (oturum 9)".
