...
export function ShareMenu({ title, url, text, imageUrl }: ShareMenuProps) {
  ...
  // Pinterest create-pin intent. Pinterest'in scraper'i `url`daki sayfayi
  // okur (Recipe/Article JSON-LD'den rich pin cikarir), `media` pin'in
  // onizleme gorseli, `description` kullanici editleyebildigi default
  // caption. imageUrl yoksa Pinterest scraper'a birakiriz.
      ...
      {!hasNativeShare && isOpen && (
        ...
        >
          ...
          >
            <span
              aria-hidden="true"
              className="flex h-8 w-8 items-center justify-center rounded-full bg-[#E60023] text-white"
            >
              <svg
                width="16"
                height="16"
