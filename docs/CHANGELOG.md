...
> Son güncelleme: **Oturum 15 sonu (23 Nis 2026), 50+ commit, en yoğun altyapı + içerik günü.** **2872 tarif prod** (2772 → +100 Mod A 30a/30b). **26 blog yazı** (25 → 26, Ramazan Sofrası mutfak-rehberi, kategori 11/7/8). **Mod E pipeline kapandı**: B14-B29 apply, toplam B1-B29 = **2900 tarif step revize** (~%95 kalite-düşük catalog). **Mod A 30a + 30b apply**: 100 yeni tarif, 50/50 yarı batch pattern kuruldu. **Neon → Vercel Marketplace migration** ($240/yıl tasarruf, Vercel-managed Neon, $20/ay Pro credit içinde erir). **Vercel Analytics** aktif (Speed Insights kaldırıldı, $10/ay sabit). **Hero A/B durduruldu** (default A, launch sonrası reenable). **Tailwind 4 dark variant [data-theme] fix**. Mod B Backfill-01 + 02 apply (200 çeviri), 03/04/05 CSV hazır (235 kalan). CODEX_BRIEF §14.5 + §14.7 B15 v1 + B16 dersleri (UTF-8 no-BOM + cümle tekrar yasağı). `gen-modb-backfill-csv.ts --start N` flag. `src/lib/prisma.ts` VERCEL_ENV runtime URL seçimi. Allergen guard ~27 spot fix. Detaylı akış: `docs/PROJECT_STATUS.md` → "23 Nisan 2026 (oturum 15)".
